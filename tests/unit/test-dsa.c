...
static void do_single_task(void)
{
    ...
    buffer_is_zero_dsa_batch(&batch_task,
                             ...
                             page_size,
                             &result);
    g_assert(result == buffer_is_zero(buf, page_size));
}

static void test_single_zero(void)
{
    g_assert(!configure_dsa(path, 1));
...
static void test_null_task(void)
{
    if (g_test_subprocess()) {
        ...
        bool result[batch_size];

        buffer_is_zero_dsa_batch(NULL, (const void**) addrs, batch_size, page_size, result);
    } else {
...
static void test_oversized_batch(void)
{
    ...
    bool result[oversized_batch_size];
...
static void test_zero_len(void)
{
    if (g_test_subprocess()) {
        ...
        buffer_is_zero_dsa_batch(&batch_task, (const void**) &buf, 1, 0, &result);
    } else {
        g_test_trap_subprocess(NULL, 0, 0);
        g_test_trap_assert_failed();
    }
...
static void test_null_buf(void)
{
    if (g_test_subprocess()) {
        ...
        bool result;
        buffer_is_zero_dsa_batch(&batch_task, NULL, 1, page_size, &result);
    } else {
        g_test_trap_subprocess(NULL, 0, 0);
        g_test_trap_assert_failed();
    }
...
{
    g_assert(!configure_dsa(path, num_devices));
    ...
    bool results[num_devices][batch_size];
...
static void test_configure_dsa_bad_path(void)
{
    ...
    g_assert(configure_dsa(&bad_path, 1));
}
...
int main(int argc, char **argv)
{
    ...
    g_test_add_func("/dsa/null_result", test_null_result);
